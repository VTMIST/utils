...
def plot_inv(datetime):
    ...
    def _build_inventory(datetime):
        ...
        for sub in subsyslist:
            # hskp files
            filelist = {i: aalpip.generate_filelist(
                start=daycheck, system=i, subsystem=sub) for i in syslist}
            # totals = [len(filelist[sys]) for sys in filelist]
            counter = np.zeros((7, 24))
            # set the filecount map for sys2+
            for system in syslist:
                for hour in range(24):
                    counter[system, hour] = [daycheck.strftime('%Y_%m_%d_{:02}'.format(
                        hour)) in file for file in filelist[system]].count(True)
    ...
    def _build_figure(daycheck, count):
        ...
        # -- AXES SETUP ---
        # titles
        fig.suptitle('Inventory for {}'.format(
            daycheck.strftime('%Y-%m-%d')), fontsize=19)
        ...
        for ax in [ax_fg, ax_sc, ax_cases, ax_hf, ax_hskp]:
            ...
            ax.xaxis.set_ticklabels(
                ['PG0', 'PG1', 'PG2', 'PG3', 'PG4', 'PG5', 'TST'])

        # --- PLOTTING ---
        # colormap
        cmap = mcolor.LinearSegmentedColormap.from_list(
            'aal-pip', [mcolor.hex2color('#f73548'), mcolor.hex2color('#35f7a3')], N=2)
        ...
        # -- FLUXGATE --
        ax_fg.pcolormesh(count['fg'], alpha=0.7, linestyle='-', linewidth=0.05,
                         color='black', cmap=cmap, norm=mcolor.Normalize(0, normal_file_count['fg']))
        # -- SEARCHCOIL --
        ax_sc.pcolormesh(count['sc'], alpha=0.7, linestyle='-', linewidth=0.05,
                         color='black', cmap=cmap, norm=mcolor.Normalize(0, normal_file_count['sc']))
        # -- CASES --
        ax_cases.pcolormesh(count['cases'], alpha=0.7, linestyle='-', linewidth=0.05,
                            color='black', cmap=cmap, norm=mcolor.Normalize(0, normal_file_count['cases']))
        # -- HF --
        ax_hf.pcolormesh(count['hf'], alpha=0.7, linestyle='-', linewidth=0.05,
                         color='black', cmap=cmap, norm=mcolor.Normalize(0, normal_file_count['hf']))
        # -- HOUSEKEEPING --
        ax_hskp.pcolormesh(count['hskp'], alpha=0.7, linestyle='-', linewidth=0.05,
                           color='black', cmap=cmap, norm=mcolor.Normalize(0, normal_file_count['hskp']))
