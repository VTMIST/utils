...
def get_ccmc_tsyg_conj(datetime, lat, lon, SW_dyn_press=1, SW_vel=450, IMF_By=0, IMF_Bz=0, DST=1, direction='North-South'):
    ...
    try:
        model_results_html = next(line for line in response.iter_lines() if b'T01 Run Results' in line).decode('utf-8').split('<br />')
        conjugate_point_raw = next(model_results_html[i-1:i+5] for i in range(len(model_results_html)) if 'Conjugate point:' in model_results_html[i])
...
def get_omni_sw_params(datetime):
    ...
    sw_date = dt.datetime.strptime(datetime.strftime('%Y%m%d%H%M'),'%Y%m%d%H%M')
    # Get data from OMNI for the minute period of interest
    minutelies = ['BZ_GSM', 'BY_GSM', 'flow_speed', 'Pressure']
    datasets = cdas.get_datasets('istp_public', idPattern='.*1MIN', labelPattern='.*OMNI.*')
    datasetID = datasets['DatasetDescription'][0]['Id']
    variables = cdas.get_variables('istp_public', datasetID)
    MIN_data = cdas.get_data('sp_phys', datasetID, sw_date, sw_date + dt.timedelta(minutes=1), minutelies)
    # Get DST from OMNI (hourly)
    datasets = cdas.get_datasets('istp_public', idPattern='.*MRG1HR', labelPattern='.*OMNI.*')
    datasetID = datasets['DatasetDescription'][0]['Id']
    variables = cdas.get_variables('istp_public', datasetID)
    DST_data = cdas.get_data('sp_phys', datasetID, sw_date - dt.timedelta(hours=2), sw_date, ['DST1800'])
